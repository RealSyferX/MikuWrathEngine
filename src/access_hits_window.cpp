...
void AccessHitsWindow::OnPaint(Gdiplus::Graphics* g, int w, int h) {
    ...
    UI::DrawText(g, 200, yStart + 2, "Count", Theme::CLR_DIM());
    UI::DrawText(g, 280, yStart + 2, "Thread", Theme::CLR_DIM());
    if (w > 400) UI::DrawText(g, 380, yStart + 2, "Module+offset", Theme::CLR_DIM());
    yStart += rowH + 2;
    ...
    for (int i = 0; i < visibleRows; i++) {
        ...
        UI::DrawText(g, 200, y + 1, buf, Theme::CLR_YELLOW());

        snprintf(buf, sizeof(buf), "%lu", hits[idx].threadId);
        UI::DrawText(g, 280, y + 1, buf, Theme::CLR_TEXT());

        // Module-relative address
        if (w > 400 && m_pm && m_pm->IsOpen()) {
            std::string modAddr = m_pm->FormatAddress(hits[idx].instruction);
            UI::DrawText(g, 380, y + 1, modAddr.c_str(), Theme::CLR_DIM());
        }

        // Double-click to copy address to clipboard
